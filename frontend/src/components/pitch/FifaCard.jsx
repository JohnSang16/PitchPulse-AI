...
export default function FifaCard({ x, y, player, isAway = false }) {
  ...
  const accent   = isAway ? "#38bdf8" : "#a3e635"
  const bodyFill = "#0b101c"
  const headFill = "#121a2b"
  const txtMain  = hovered ? accent : (isAway ? "rgba(56,189,248,0.9)" : "rgba(163,230,53,0.9)")
  const txtName  = "#edf2fa"
  const txtDim   = isAway ? "rgba(56,189,248,0.45)" : "rgba(163,230,53,0.45)"
  const border   = hovered ? accent : (isAway ? "rgba(56,189,248,0.35)" : "rgba(163,230,53,0.35)")
  ...
  return (
    ...
    >
      ...
      <line x1={cx + 4} y1={cy + 20} x2={cx + W - 4} y2={cy + 20}
        stroke={isAway ? "rgba(56,189,248,0.2)" : "rgba(163,230,53,0.2)"} strokeWidth="0.5" />
      ...
      <line x1={cx + 4} y1={cy + 50} x2={cx + W - 4} y2={cy + 50}
        stroke={isAway ? "rgba(56,189,248,0.15)" : "rgba(163,230,53,0.15)"} strokeWidth="0.5" />
